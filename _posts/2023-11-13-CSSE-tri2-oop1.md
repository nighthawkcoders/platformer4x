...
- _sass/minima/custom-styles.sccs,_sass/nighthawk are SCSS files that defines the styles for the game, including elements like score display, game buttons, and input fields.
...
- assets/js/platformer is the directory where the game's key logic is written in JavaScript.  This can be customoized complete as no third-party libraries.
...
The core logic is great if you want to learn about the internals of the game.  However, if you want to build a GameLevel you really need to uderstand how to setup a level and how to watch and react to collisons.
...
- To make you own game level, create you own GameSet file, by copying an existing definition and start making iterative changes.  
...
To create a new game level, define a new GameSet file similar to GameSetHills.js. This file will specify the assets and objects for the level.
...
Be aware that a "class" is defined in your meta data as the Template that defines your GameObjects behavior.  There are many predefined templates that can be extended to enable unique behaviors.   It is best to star with minimal customization and then override behaviors as you obtain understanding.

Look at extings class templates that extend:  Background.js, Enemy.js, Platform.js, PlayerBase,js.
...
- Go through iterative development cycle to creat custom logic for interactions and collisions in the new classes.
